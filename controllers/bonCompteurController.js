const { initModel } = require('../models/bonCompteurModel');

// Get the current compteur
const getCompteur = async (req, res) => {
  try {
    const BonCompteur = initModel();
    if (!BonCompteur) throw new Error("Model not initialized");

    let compteur = await BonCompteur.findOne();
    
    // If no compteur exists, create one with current year
    if (!compteur) {
      const currentYear = new Date().getFullYear().toString().slice(-2);
      compteur = await BonCompteur.create({ 
        year: currentYear, 
        comptValue: '1',
        updatedAt: new Date() 
      });
